...
d={} # format key=table string, value = list of file paths
tableType={} #dict for types of tables, keyed by number of columns
...
for elt in fileL: 
    readFile= "../data/warfarin/"+elt #this is the file I want to read
    f = open(readFile)
    ...
    #****************************************************************
        ...
        # approach one, tables are typed by column numbers
        i = -1 
        for td in cols:
            i += 1 #+1 for every column found
        
        if i in tableType.keys():
...
print "Number of unique table strings: %s" % len(d.keys())

s = "<HTML>"
for k,v in d.iteritems():
    s += "<BR><HLINE><BR>"
    s += "<PRE>Files with this table string: %s</PRE><BR>" % v
    s += k
